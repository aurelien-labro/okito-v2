      ...
      <div className="relative h-full w-full max-w-md overflow-y-auto bg-white p-6 shadow-xl">
        <div className="mb-4 flex items-start justify-between">
          <div>
            ...
            <p className="font-mono text-xs text-stone-400">{phone}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
            ...
            <div className="space-y-3">
              {profile.timeline.map((t, i) => (
                <div key={`${t.at}-${i}`} className="flex gap-3">
                  <span
                    className={`ti ${TIMELINE_ICON[t.kind] ?? "ti-point"} mt-0.5 text-[15px] text-stone-400`}
                    aria-hidden="true"
                  />
                  <div className="min-w-0 flex-1">
                    <div className="text-sm">{t.title}</div>
                    {t.detail && <div className="text-xs text-stone-500">{t.detail}</div>}
        ...
        {!profile && !err && <p className="text-sm text-stone-400">Chargement…</p>}
      </div>
    </div>
  );
}
...
function MiniStat({ label, value, warn }: { label: string; value: string; warn?: boolean }) {
  return (
    <div className="rounded-lg bg-stone-100/70 p-2">
      ...
      <div className={`text-lg font-medium ${warn ? "text-rose-700" : "text-stone-900"}`}>
        {value}
      </div>
    </div>
  );
